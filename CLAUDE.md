...
You are building **Svane** — a runes-native, SvelteKit-first data fetching & caching library.
...
Svane has a **pure TypeScript core** and a **thin Svelte adapter**:
...
- Do NOT add `QueryClientProvider` or any wrapper component — Svane uses explicit `createCache()`
...
- Do NOT add dependencies unless absolutely necessary — Svane should be zero-dep
- Do NOT use `createQuery()` naming — Svane uses `cache.query()` to differentiate from TanStack
